...
class DomainContext(BaseModel):
    ...
    domain: str = Field(
        description="Target application domain (e.g. finance, healthcare)"
    )
    sensitive_entities: list[str] = Field(
        default_factory=list, description="Sensitive data entities found"
    )
    dangerous_tools: list[str] = Field(
        default_factory=list, description="Tools with high-risk capabilities"
    )
    business_rules: list[str] = Field(
        default_factory=list, description="Business rules that could be exploited"
    )
...
class AgentInfo(BaseModel):
    ...
    system_prompt: str = Field(
        default="", description="Agent system prompt if discovered"
    )
...
class RAGSurface(BaseModel):
    ...
    vectorstore_type: str = Field(
        description="Type of vector store (e.g. Qdrant, Chroma)"
    )
...
class MemorySurface(BaseModel):
    ...
    shared: bool = Field(
        default=False, description="Whether memory is shared across agents"
    )
    file_path: str = Field(default="", description="Source file path")
...
class PipelineInfo(BaseModel):
    ...
    agents: list[str] = Field(
        default_factory=list, description="Agents in this pipeline"
    )
    flow_type: str = Field(
        default="sequential", description="Flow type: sequential, parallel, conditional"
    )
...
class GuardrailInfo(BaseModel):
    ...
    guardrail_type: str = Field(
        default="input", description="Type: input, output, tool_call"
    )
...
class OTelCoverage(BaseModel):
    ...
    tracing_enabled: bool = Field(
        default=False, description="Whether OTel tracing is set up"
    )
    traced_components: list[str] = Field(
        default_factory=list, description="Components with tracing"
    )
    untraced_components: list[str] = Field(
        default_factory=list, description="Components without tracing"
    )
    coverage_pct: float = Field(
        default=0.0, description="Percentage of components traced"
    )
...
class AttackRecommendation(BaseModel):
    ...
    suggested_payloads: list[str] = Field(
        default_factory=list, description="Example payloads to try"
    )
...
class ThreatModel(BaseModel):
    ...
    agents_found: list[AgentInfo] = Field(
        default_factory=list, description="Discovered agents"
    )
    rag_surfaces: list[RAGSurface] = Field(
        default_factory=list, description="RAG attack surfaces"
    )
    memory_surfaces: list[MemorySurface] = Field(
        default_factory=list, description="Memory attack surfaces"
    )
    tool_surfaces: list[ToolSurface] = Field(
        default_factory=list, description="Tool attack surfaces"
    )
    pipeline_map: list[PipelineInfo] = Field(
        default_factory=list, description="Pipeline/flow map"
    )
    guardrails: list[GuardrailInfo] = Field(
        default_factory=list, description="Discovered guardrails"
    )
    otel_coverage: OTelCoverage = Field(description="Observability coverage assessment")
    recommended_attacks: list[AttackRecommendation] = Field(
        default_factory=list, description="Recommended attacks"
    )
...
class Payload(BaseModel):
    ...
    domain_adapted: bool = Field(
        default=False, description="Whether payload was domain-adapted"
    )
...
class Vulnerability(BaseModel):
    ...
    evidence: str = Field(
        default="", description="Evidence/output demonstrating the vulnerability"
    )
...
class AttackResult(BaseModel):
    ...
    payloads_tried: list[Payload] = Field(
        default_factory=list, description="Payloads attempted"
    )
    vulnerabilities_found: list[Vulnerability] = Field(
        default_factory=list, description="Vulnerabilities discovered"
    )
    overall_severity: str = Field(
        default="None", description="Overall severity for this attack type"
    )
    success_rate: float = Field(
        default=0.0, description="Fraction of payloads that succeeded"
    )
    execution_time_seconds: float = Field(
        default=0.0, description="Time taken in seconds"
    )
...
class ChaosReport(BaseModel):
    ...
    vulnerability_count: VulnCount = Field(
        default_factory=VulnCount, description="Vulnerability counts"
    )
    otel_coverage_pct: float = Field(
        default=0.0, description="OTel coverage percentage"
    )
    blind_spots: list[str] = Field(
        default_factory=list, description="Identified blind spots"
    )
